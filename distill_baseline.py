...
def main(args):
    ...
    # 创建目录保存数据
    save_dir = "exp_results_2024cvpr"
    os.makedirs(save_dir, exist_ok=True)
    loss_history = []
    
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    loss_png_path = os.path.join(save_dir, f"loss_{timestamp}.png")
    ...
    wandb.init(sync_tensorboard=False,
               ...
               job_type="CleanRepo",
               config=args,
               name = f'{args.dataset}_ipc{args.ipc}_{args.lr_img}_{datetime.now().strftime("%Y%m%d%H%M%S")}'
               )
    ...
    optimizer_img = torch.optim.SGD([image_syn, ], lr=args.lr_img, momentum=0.8) # optimizer_img for synthetic data
    ...
    if args.method == "MTT":
        ...
        for it in trange(0, args.Iteration+1):
            ...
            #if it in eval_it_pool and (save_this_best_ckpt or it % 1000 == 0):
            if it==100000 and (save_this_best_ckpt or it % 1000 == 0):
                image_save = image_syn.detach()
                save_dir = os.path.join(args.save_path, project_name, wandb.run.name)
                if not os.path.exists(save_dir):
                    os.makedirs(save_dir)
            ...
            for c in range(0,num_classes):
                ...
                t=None
                loss+=30 * cfloss(output_real, output_syn, t,args)
            ...
            #logger.info("计算Loss前video_syn.grad 是否为 None:"+ str(image_syn.grad is None)) 
            print(f"现在是第{it}个iteration，loss_avg是{loss_avg}")
            wandb.log({"Loss": loss_avg}, step=it)
   
   
    plt.figure(figsize=(8, 6))
    plt.plot(loss_history, label="iteration Loss", color='blue', linewidth=2)
