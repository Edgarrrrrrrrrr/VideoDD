...
class HMDB51(tdata.Dataset):
    def __init__(self, path, split, transform):
        ...
        self.video_dirs = []  #存储的是视频路径
        #self.video_dirs = [
        #"/path/to/dataset/jpegs_112/video1",
        #"/path/to/dataset/jpegs_112/video2",
        #"/path/to/dataset/jpegs_112/video3"]
        ...
        with open(csv_path) as fp:
            ...
            for item in reader:
                ...
                self.class_strs.add(item["label"])
                self.video_dirs.append(sample_dir)
        #print(len(self.video_dirs))
        self.class_strs = sorted(self.class_strs)  #self.class_strs = ["jump", "run", "walk"]
        #print("Number of classes (self.class_strs):", len(self.class_strs))
        self.class_2_idx = {x: i for i, x in enumerate(self.class_strs)} #self.class_2_idx = {"jump": 0, "run": 1, "walk": 2}
        #print(type(self.class_2_idx))
        self.labels = [self.class_2_idx[l] for l in self.label_strs]  #表示每个样本的标签
        #print("Number of labels (self.labels):", len(self.labels))
        self.targets = self.labels  # alias
